...
// PUT Method Update the User record
router.put("/profile/password", jwtAuthMiddleware, async (req, res) => {
  try {
    const userId = req.params.id; // Extract the id from the token
    const { currentPasswrod, newPassword } = req.body; // Extract current and new passwords from request body
    ...
    // Find the use by useId
    const user = await User.findById(userId);

    // If user does not exist or password does not match, return error
    if (!user || !(await user.comparePassword(currentPasswrod))) {
      return res.status(401).json({ error: "Invalid current Passwrod" });
    }
